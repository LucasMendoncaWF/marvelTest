...
const publicKey = "8f0bb5f53cb83a7189e960ad214f91dc";
const privateKey = "484f703ca97ef229487bf3b71ba1ce3f1af76e73";
const urlMarvelAPI = `${window.location.protocol}//gateway.marvel.com`;
...
class MarvelCharacterLoader {
  ...
  static getMarvelCharacters(origOptions = {}) {
    ...
    const toMD5 = stamp + privateKey + publicKey;
    const hashRequired = CryptoJS.MD5(toMD5).toString(CryptoJS.enc.Hex);

    const offset = pageOptions.quantity * (pageOptions.page - 1);

    return fetch(`${urlMarvelAPI}/v1/public/characters?apikey=${publicKey}&ts=${stamp}&offset=${offset}&limit=${pageOptions.quantity}&hash=${hashRequired}`);
  }
